...
export const exportDashboardToPDF = async (
  ...
  try {
    ...
    const addWrappedText = (text, x, y, maxWidth, fontSize = 11, fontWeight = 'normal') => {
    ...
    pdf.text('Analisis Komprehensif Kesehatan Tidur, Stress, dan Aktivitas Fisik', margin, yPosition + 35);

    pdf.setFontSize(10);
    pdf.setTextColor(80, 80, 80);
    pdf.text(`Created by: AIKO HANAKO LASUT & PRINCE AZARYA TIMOTHY TAMPI`, margin, yPosition + 50);
    pdf.text(`Generated on: ${new Date().toLocaleDateString('id-ID', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      hour: '2-digit',
    ...
    const extractDashboardData = () => {
      ...
      // Get KPI data
      const kpis = [];
      const kpiElements = document.querySelectorAll('.kpi-card, [class*="kpi"]');
      kpiElements.forEach(kpi => {
        const text = kpi.textContent || '';
        if (text.includes('Durasi Tidur') || text.includes('Kualitas Tidur') ||
            text.includes('Tingkat Stres') || text.includes('Langkah Harian') ||
            text.includes('Detak Jantung') || text.includes('Gangguan Tidur')) {
          kpis.push(text.trim());
        }
      });
      ...
      if (kpis.length === 0) {
        const kpiSection = document.querySelector('[class*="KPI"], [class*="kpi"]');
        if (kpiSection) {
          const kpiItems = kpiSection.querySelectorAll('*');
          kpiItems.forEach(item => {
            const text = item.textContent || '';
            if (text.length > 10 && text.length < 100 &&
                (text.includes('jam') || text.includes('/10') || text.includes('bpm') ||
                 text.includes('langkah') || text.includes('%'))) {
              kpis.push(text.trim());
            }
          });
        }
      ...
      setTimeout(() => {
        ...
        // Averages
        const avgElements = document.querySelectorAll('.accordion-body [style*="border-left: 3px solid"]');
        avgElements.forEach(el => {
          const text = el.textContent?.trim() || '';
          if (text.includes('jam') || text.includes('/10') || text.includes('level') || text.includes('langkah')) {
            insights.averages.push(text);
          }
        });

        // Statistics
        const statElements = document.querySelectorAll('.accordion-body [style*="border-left: 3px solid #17a2b8"]');
        statElements.forEach(el => {
          const text = el.textContent?.trim() || '';
          if (text.includes('%') || text.includes('tahun') || text.includes('Laki-laki')) {
            insights.statistics.push(text);
          }
        });

        // Main Insights
        const insightElements = document.querySelectorAll('.accordion-body [style*="border-left: 4px solid #00aa44"]');
        insightElements.forEach(el => {
          const text = el.textContent?.trim() || '';
          if (text.length > 20 && !text.includes('jam') && !text.includes('/10')) {
            insights.mainInsights.push(text);
          }
        });

        // Trends
        const trendElements = document.querySelectorAll('.accordion-body [style*="border-left: 4px solid #ff9900"]');
        ...
        // Recommendations
        const recElements = document.querySelectorAll('.accordion-body [style*="border-left: 4px solid #dc3545"]');
    ...
    pdf.setFont('helvetica', 'normal');
    pdf.setTextColor(60, 60, 60);
    pdf.text(`Dataset terdiri dari ${insights.totalData || '374'} responden dengan berbagai variabel kesehatan tidur.`, margin, yPosition);
    yPosition += 10;
    ...
    // Main Insights
    checkNewPage(60);
    yPosition = addSectionHeader('Apa insight utama dari data?', '💡', yPosition);

    if (insights.mainInsights && insights.mainInsights.length > 0) {
      insights.mainInsights.forEach((insight, index) => {
        yPosition = addWrappedText(`${index + 1}. ${insight}`, margin + 5, yPosition, contentWidth - 10, 10);
        yPosition += 3;
      });
    } else {
    ...
    if (insights.trends && insights.trends.length > 0) {
      insights.trends.forEach((trend, index) => {
        yPosition = addWrappedText(`${index + 1}. ${trend}`, margin + 5, yPosition, contentWidth - 10, 10);
        yPosition += 3;
      });
    } else {
      const defaultTrends = [
    ...
    // Recommendations
    checkNewPage(60);
    yPosition = addSectionHeader('Apa rekomendasi berdasarkan data?', '🎯', yPosition);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      pdf.text(`Sleep Health & Lifestyle Dashboard Report - Page ${i} of ${pageCount}`, margin, pageHeight - 10);
      pdf.text(`Generated: ${new Date().toLocaleDateString('id-ID')}`, pageWidth - margin - 40, pageHeight - 10);
    }
